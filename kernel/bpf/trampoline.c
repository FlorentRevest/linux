			    ...
			    struct pt_regs *regs)
{
	...
	struct bpf_fprobe_call_context call_ctx;
	struct bpf_tramp_link *link;
	struct hlist_node *tmp;

	call_ctx.args[call_ctx.args_cnt] = regs_return_value(regs);

	hlist_for_each_entry_safe(link, tmp, &tr->progs_hlist[BPF_TRAMP_FEXIT], tramp_hlist)
		call_bpf_prog(link, &call_ctx.ctx, call_ctx.args);
}

static void bpf_fprobe_entry(struct fprobe *fp, unsigned long ip,
			    struct pt_regs *regs)
{
	struct bpf_trampoline *tr = container_of(fp, struct bpf_trampoline, probe);
	struct bpf_fprobe_call_context call_ctx;
	...
	for (i = 0; i < call_ctx.args_cnt; i++)
		call_ctx.args[i] = regs_get_kernel_argument(regs, i);

	hlist_for_each_entry_safe(link, tmp, &tr->progs_hlist[BPF_TRAMP_FENTRY], tramp_hlist)
		call_bpf_prog(link, &call_ctx.ctx, call_ctx.args);

	call_ctx.args[call_ctx.args_cnt] = 0;
	hlist_for_each_entry_safe(link, tmp, &tr->progs_hlist[BPF_TRAMP_MODIFY_RETURN], tramp_hlist) {
...
static int bpf_trampoline_update(struct bpf_trampoline *tr,
				 bool lock_direct_mutex)
{
	bool registered = tr->probe.ops.flags & FTRACE_OPS_FL_ENABLED;
